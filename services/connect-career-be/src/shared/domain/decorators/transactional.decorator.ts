...
export function Transactional(
    ...
    return function (
        ...
        const logger = new Logger(`Transactional:${target.constructor.name}.${propertyKey}`);
        ...
        descriptor.value = async function (...args: any[]) {
            ...
            try {
                logger.debug(`Beginning transaction with isolation level: ${isolationLevel || 'default'}`);
