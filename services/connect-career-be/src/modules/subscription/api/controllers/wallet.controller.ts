...
import { TopUpWalletDto, TransactionDto, UsageHistoryDto, WalletBalanceResponseDto, WalletStatisticsDto } from '../dtos/wallet.dto';
...
export class WalletController {
  ...
  @Get('balance')
  @ApiOperation({ summary: 'Get wallet balance with transaction history and usage' })
  @ApiResponse({ 
    status: 200, 
  ...
  async getBalance(
    ...
    const [recentTransactions, recentUsageHistory, statistics] = await Promise.all([
    ...
    const transactionDtos: TransactionDto[] = recentTransactions.map((t): TransactionDto => ({
      id: t.id,
      type: t.type,
    ...
    const usageHistoryDtos: UsageHistoryDto[] = recentUsageHistory.map((u): UsageHistoryDto => ({
      id: u.id,
