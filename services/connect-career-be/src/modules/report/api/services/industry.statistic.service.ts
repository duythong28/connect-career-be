...
export class IndustryStatisticService {
  ...
  async getIndustryStatistics(
    ...
    const cachedResult =
      await this.cache.get<IndustryStatisticItem[]>(cacheKey);
  ...
  async getIndustryStatisticsByName(): Promise<IndustryNameStatisticItem[]> {
    ...
    const cachedResult =
      await this.cache.get<IndustryNameStatisticItem[]>(cacheKey);
    if (cachedResult && cachedResult.length > 0) {
    ...
    const statisticsWithTime: IndustryNameStatisticItem[] = statistics.map(
      (item) => ({
        key: item.key,
