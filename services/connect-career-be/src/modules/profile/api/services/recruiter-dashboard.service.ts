...
import { OrganizationMembership, MembershipStatus } from '../../domain/entities/organization-memberships.entity';
import { Application, ApplicationStatus } from 'src/modules/applications/domain/entities/application.entity';
...
import { RecruiterDashboardQueryDto, DashboardPeriod } from '../dtos/recruiter-dashboard.dto';
import { CompanyOverview, MyWorkSummary, QuickActions, RecruiterDashboardResponse, RecruiterPerformance, UpcomingTasks } from '../dtos/recruiter-dashboard.response.dto';
@Injectable()
export class RecruiterDashboardService {
  ...
  async getRecruiterDashboard(
    ...
    try {
      ...
      if (!organization) {
        this.logger.error(`Membership ${targetMembership.id} has no organization`);
      ...
      const [
        ...
        this.getQuickActions(userId, organizationId, targetMembership.role.name),
        this.getRecruiterPerformance(userId, organizationId, startDate, endDate, query),
      ]);
      ...
      this.logger.error(`Error getting recruiter dashboard for user ${userId}:`, error.stack);
      throw error; // Re-throw to let NestJS handle it
    }
  }

  private calculateDateRange(query: RecruiterDashboardQueryDto): { startDate: Date; endDate: Date } {
  ...
  private async getMyWorkSummary(
    ...
    // Group by status
    const byStatus = assignedApplications.reduce((acc, app) => {
    ...
    const needsAttention = assignedApplications.filter(app => {
      ...
      if (app.lastContactDate) {
        const daysSinceContact = Math.floor(
          (new Date().getTime() - new Date(app.lastContactDate).getTime()) / (1000 * 60 * 60 * 24)
        );
    ...
    const interviewsToday = myInterviews.filter(i => {
      ...
      return interviewDate >= todayStart && interviewDate < new Date(todayStart.getTime() + 24 * 60 * 60 * 1000);
    }).length;
    ...
    const upcomingInterviews = myInterviews.filter(i => {
      const interviewDate = new Date(i.scheduledDate);
      return interviewDate >= now && [InterviewStatus.SCHEDULED, InterviewStatus.RESCHEDULED].includes(i.status);
    }).length;
    ...
    const pendingOffers = myOffers.filter(o => o.status === OfferStatus.PENDING).length;
    const awaitingResponseOffers = myOffers.filter(
      o => o.status === OfferStatus.PENDING || o.status === OfferStatus.NEGOTIATING
    ).length;
    const acceptedOffers = myOffers.filter(o => o.status === OfferStatus.ACCEPTED).length;
    const rejectedOffers = myOffers.filter(o => o.status === OfferStatus.REJECTED).length;

    // Get tasks (reminders)
    const allReminders = assignedApplications
      .flatMap(app => (app.reminders || []).map(r => ({ ...r, applicationId: app.id })))
      .filter(r => !r.completed);

    const nowDate = new Date();
    const overdueTasks = allReminders.filter(r => new Date(r.dueDate) < nowDate).length;
  ...
  private async getCompanyOverview(
    ...
    const pausedJobs = jobs.filter(j => j.status === JobStatus.PAUSED).length;
    ...
    const byStage = Array.from(pipelineStages.entries()).map(([stageKey, count]) => ({
      stageName: applications.find(a => (a.currentStageKey || a.status) === stageKey)?.currentStageName || stageKey,
      stageKey,
      count,
    }));
    ...
    return {
      ...
      this.logger.error(`Error getting company overview for organization ${organizationId}:`, error.stack);
      throw error;
    }
  }
  ...
  private async getUpcomingTasks(
    ...
    const interviews = upcomingInterviews.map(interview => {
      ...
      const isToday = interviewDate >= todayStart && interviewDate < new Date(todayStart.getTime() + 24 * 60 * 60 * 1000);
      const isUpcoming = interviewDate >= now && interviewDate <= weekEnd;

      return {
        id: interview.id,
        applicationId: interview.applicationId,
        candidateName: interview.application?.candidate?.fullName || 'Unknown',
    ...
    const followUps = assignedApplications
      .filter(app => {
        ...
        const daysSince = Math.floor(
          (now.getTime() - new Date(app.lastContactDate).getTime()) / (1000 * 60 * 60 * 24)
        );
        return daysSince >= 7 && ![ApplicationStatus.HIRED, ApplicationStatus.REJECTED, ApplicationStatus.WITHDRAWN].includes(app.status);
      })
      .map(app => ({
        applicationId: app.id,
        candidateName: app.candidate?.fullName || 'Unknown',
        ...
        daysSinceLastContact: app.lastContactDate
          ? Math.floor((now.getTime() - new Date(app.lastContactDate).getTime()) / (1000 * 60 * 60 * 24))
          : 999,
        status: app.status,
        needsFollowUp: true,
      }))
    ...
    const pendingActions = assignedApplications
      .filter(app => {
        ...
        if ([ApplicationStatus.NEW, ApplicationStatus.UNDER_REVIEW].includes(app.status)) {
          return true;
        }
        // Applications ready for interview scheduling
        if (app.status === ApplicationStatus.SHORTLISTED && !app.nextInterviewDate) {
          return true;
        }
        // Applications ready for offer
        if (app.status === ApplicationStatus.INTERVIEW_COMPLETED && !app.currentOfferId) {
          return true;
        }
        return false;
      })
      .map(app => {
        ...
        if (app.status === ApplicationStatus.SHORTLISTED) actionType = 'schedule_interview';
        if (app.status === ApplicationStatus.INTERVIEW_COMPLETED) actionType = 'send_offer';
    ...
    return {
      ...
      this.logger.error(`Error getting upcoming tasks for user ${userId}:`, error.stack);
      throw error;
    }
  }
  ...
  private async getQuickActions(
    ...
    const canCreateJob = ['owner', 'admin', 'hr_manager', 'recruiter'].includes(roleName);
    const canInviteCandidate = ['owner', 'admin', 'hr_manager', 'recruiter'].includes(roleName);
    const canScheduleInterview = ['owner', 'admin', 'hr_manager', 'recruiter'].includes(roleName);
    ...
    const applicationsToReview = assignedApplications.filter(
      app => [ApplicationStatus.NEW, ApplicationStatus.UNDER_REVIEW].includes(app.status)
    ).length;

    const interviewsToSchedule = assignedApplications.filter(
      app => app.status === ApplicationStatus.SHORTLISTED && !app.nextInterviewDate
    ).length;

    const offersToSend = assignedApplications.filter(
      app => app.status === ApplicationStatus.INTERVIEW_COMPLETED && !app.currentOfferId
    ).length;
  ...
  private async getRecruiterPerformance(
    ...
    const applicationsReviewed = applications.filter(
      app => app.reviewedBy === userId && app.status !== ApplicationStatus.NEW
    ).length;
    ...
    const offerAcceptanceRate = offersSent > 0 ? (acceptedOffers / offersSent) * 100 : 0;

    // Get hires
    const hires = applications.filter(app => app.status === ApplicationStatus.HIRED).length;

    // Calculate average times
    const reviewedApplications = applications.filter(app => app.reviewedBy === userId && app.reviewedAt);
    const averageTimeToReview = reviewedApplications.length > 0
      ? reviewedApplications.reduce((sum, app) => {
          const appliedDate = new Date(app.appliedDate);
          const reviewedDate = app.reviewedAt ? new Date(app.reviewedAt) : new Date();
          const hours = (reviewedDate.getTime() - appliedDate.getTime()) / (1000 * 60 * 60);
          return sum + hours;
        }, 0) / reviewedApplications.length
      : 0;

    const scheduledInterviews = interviews.filter(i => i.status === InterviewStatus.SCHEDULED || i.status === InterviewStatus.COMPLETED);
    const averageTimeToSchedule = scheduledInterviews.length > 0
      ? scheduledInterviews.reduce((sum, interview) => {
          ...
          const hours = (scheduledDate.getTime() - appliedDate.getTime()) / (1000 * 60 * 60);
          return sum + hours;
        }, 0) / scheduledInterviews.length
      : 0;

    // Response rate (candidate responses to communications)
    const applicationsWithContact = applications.filter(app => app.lastCandidateResponseDate);
    ...
    if (query.period) {
      ...
      comparison = {
        ...
        changes: {
          applicationsReviewed: {
            change: metrics.applicationsReviewed - previousMetrics.metrics.applicationsReviewed,
            percentage: previousMetrics.metrics.applicationsReviewed > 0
              ? ((metrics.applicationsReviewed - previousMetrics.metrics.applicationsReviewed) / previousMetrics.metrics.applicationsReviewed) * 100
              : 0,
          },
          hires: {
            change: metrics.hires - previousMetrics.metrics.hires,
            percentage: previousMetrics.metrics.hires > 0
              ? ((metrics.hires - previousMetrics.metrics.hires) / previousMetrics.metrics.hires) * 100
              : 0,
          },
        },
      };
