...
import { FileQueryOptions, IFileRepository } from "./interfaces/file.repository.interface";
...
export class FileRepository implements IFileRepository {
    ...
    async findMany(options: FileQueryOptions = {}): Promise<{ files: File[]; total: number }> {
    ...
    async findByUser(user: User, options: Omit<FileQueryOptions, 'uploadedBy'> = {}): Promise<{ files: File[]; total: number }> {
        return this.findMany({ ...options, uploadedBy: user.id });
    }

    async findByFolder(folder: string, options: Omit<FileQueryOptions, 'folder'> = {}): Promise<{ files: File[]; total: number }> {
        return this.findMany({ ...options, folder });
    }

    async findByTags(tags: string[], options: Omit<FileQueryOptions, 'tags'> = {}): Promise<{ files: File[]; total: number }> {
