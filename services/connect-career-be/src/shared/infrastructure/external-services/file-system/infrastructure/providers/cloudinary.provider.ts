...
import { IFileSystemService, FileUploadResult, SignedUploadParams } from "../interfaces/file-system.interface";
...
export class CloudinaryProvider implements IFileSystemService {
    ...
    async upload(file: Express.Multer.File, params: SignedUploadParams = {}): Promise<FileUploadResult> {
        try {
            const uploadResult = await new Promise<FileUploadResult>((resolve, reject) => {
                const uploadOptions: CloudinaryUploadOptions = {
                    ...
                    transformation: params.transformation as Record<string, unknown> | string | undefined,
                    tags: params.tags,
    ...
    async uploadFromUrl(url: string, params: SignedUploadParams = {}): Promise<FileUploadResult> {
        try {
            const uploadOptions: CloudinaryUploadOptions = {
                folder: params.folder || 'uploads',
                public_id: params.public_id,
            ...
            const result = await cloudinary.uploader.upload(url, uploadOptions) as CloudinaryUploadResult;
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
            throw new BadRequestException(`Failed to upload from URL: ${errorMessage}`);
        }
    }
    ...
    generateSignedUploadUrl(params: SignedUploadParams = {}): {
        ...
        const publicId = params.public_id || `file_${timestamp}_${Math.random().toString(36).substr(2, 9)}`;
        ...
        if (!apiSecret || !cloudName || !apiKey) {
            throw new Error('Missing required Cloudinary configuration for signed upload');
        }
