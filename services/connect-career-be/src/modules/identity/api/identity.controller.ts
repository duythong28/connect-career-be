...
export class IdentityController {
  ...
  @ApiOperation({ summary: 'User login with username/password and optional MFA' })
  ...
  async login(@Body() loginDto: LoginDto, @Request() req): Promise<AuthTokensResponseDto> {
    const deviceInfo = {
      ...loginDto,
      ipAddress: req.ip,
  ...
  @ApiResponse({ status: 400, description: 'Registration failed - user exists or invalid data' })
  ...
  @ApiResponse({ status: 200, description: 'Token refreshed successfully', type: AuthTokensResponseDto })
  @ApiResponse({ status: 401, description: 'Invalid refresh token' })
  async refreshTokens(@Body() refreshTokenDto: RefreshTokenDto): Promise<AuthTokensResponseDto> {
