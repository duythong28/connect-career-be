...
import { Application, ApplicationStatus, ApplicationSource } from 'src/modules/applications/domain/entities/application.entity';
...
import { BaseQueryConditions, ConversionRateMetrics, HiringEffectivenessQueryDto, HiringEffectivenessSummary, HiringMetricsPeriod, PipelineMetrics, SourceEffectivenessMetrics, TimeToHireMetrics } from '../dtos/hiring-effectiveness.dto';
...
export class OrganizationHiringAnalyticsService {
  ...
  async getHiringEffectiveness(
    ...
    // Calculate metrics
    const overview = this.calculateOverview(applications, jobs, interviews, offers, hiredApplications);
    const timeMetrics = await this.calculateTimeMetrics(applications, hiredApplications, jobs, query);
    const conversionRates = this.calculateConversionRates(applications, interviews, offers, hiredApplications);
    const sourceEffectiveness = this.calculateSourceEffectiveness(applications, hiredApplications);
    const pipelineMetrics = this.calculatePipelineMetrics(applications);
    const qualityMetrics = this.calculateQualityMetrics(applications, interviews, offers, hiredApplications);
    ...
    if (query.compareWithPrevious && previousPeriod) {
      const previousMetrics = await this.getHiringEffectiveness(organizationId, {
        ...query,
  ...
  private calculateDateRange(query: HiringEffectivenessQueryDto): { startDate: Date; endDate: Date } {
  ...
  private getStartDateForPeriod(endDate: Date, period: HiringMetricsPeriod): Date {
  ...
  private calculatePreviousPeriod(startDate: Date, endDate: Date): { startDate: Date; endDate: Date } {
  ...
  private async getApplications(conditions: BaseQueryConditions) {
    const queryBuilder = this.applicationRepository
      ...
      .where('job.organizationId = :organizationId', { organizationId: conditions.organizationId })
      .andWhere('application.appliedDate >= :startDate', { startDate: conditions.dateRange.startDate })
      .andWhere('application.appliedDate <= :endDate', { endDate: conditions.dateRange.endDate });

    if (conditions.jobId) {
      queryBuilder.andWhere('application.jobId = :jobId', { jobId: conditions.jobId });
    }
  ...
  private async getJobs(conditions: BaseQueryConditions) {
    const queryBuilder = this.jobRepository
      ...
      .where('job.organizationId = :organizationId', { organizationId: conditions.organizationId })
      .andWhere('job.createdAt >= :startDate', { startDate: conditions.dateRange.startDate })
      .andWhere('job.createdAt <= :endDate', { endDate: conditions.dateRange.endDate });
  ...
  private async getInterviews(conditions: BaseQueryConditions) {
    const queryBuilder = this.interviewRepository
      ...
      .innerJoin('application.job', 'job')
      .where('job.organizationId = :organizationId', { organizationId: conditions.organizationId })
      .andWhere('interview.scheduledDate >= :startDate', { startDate: conditions.dateRange.startDate })
      .andWhere('interview.scheduledDate <= :endDate', { endDate: conditions.dateRange.endDate });
  ...
  private async getOffers(conditions: BaseQueryConditions) {
    const queryBuilder = this.offerRepository
      ...
      .innerJoin('application.job', 'job')
      .where('job.organizationId = :organizationId', { organizationId: conditions.organizationId })
      .andWhere('offer.createdAt >= :startDate', { startDate: conditions.dateRange.startDate })
      .andWhere('offer.createdAt <= :endDate', { endDate: conditions.dateRange.endDate });
  ...
  private calculateOverview(applications: Application[], jobs: Job[], interviews: Interview[], offers: Offer[], hiredApplications: Application[]) {
    const activeJobs = jobs.filter(job => job.status === JobStatus.ACTIVE).length;
    const activeCandidates = applications.filter(
      app => ![ApplicationStatus.HIRED, ApplicationStatus.REJECTED, ApplicationStatus.WITHDRAWN].includes(app.status)
    ).length;
  ...
  private async calculateTimeMetrics(
    ...
    const timeToHireValues = hiredApplications
      .map(app => {
        ...
        const hiredDate = app.lastStatusChange ? new Date(app.lastStatusChange) : new Date();
        return Math.floor((hiredDate.getTime() - appliedDate.getTime()) / (1000 * 60 * 60 * 24));
      })
      .filter(days => days >= 0);

    const average = timeToHireValues.length > 0
    ...
    const byJob = await Promise.all(
      jobs.map((job) => {
        const jobHires = hiredApplications.filter(app => app.jobId === job.id);
        if (jobHires.length === 0) return null;

        const avgDays = jobHires.reduce((sum, app) => {
          const appliedDate = new Date(app.appliedDate);
          const hiredDate = app.lastStatusChange ? new Date(app.lastStatusChange) : new Date();
          return sum + Math.floor((hiredDate.getTime() - appliedDate.getTime()) / (1000 * 60 * 60 * 24));
        }, 0) / jobHires.length;
  ...
  private calculateTrend(hiredApplications: Application[], query: HiringEffectivenessQueryDto) {
    ...
    hiredApplications.forEach(app => {
      ...
      const appliedDate = new Date(app.appliedDate);
      const days = Math.floor((date.getTime() - appliedDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  private calculateConversionRates(
    ...
    const totalOffers = offers.filter(o => o.status !== OfferStatus.REJECTED).length;
    const totalHires = hiredApplications.length;

    const applicationToInterview = totalApplications > 0
      ? (totalInterviews / totalApplications) * 100
      : 0;

    const interviewToOffer = totalInterviews > 0
      ? (totalOffers / totalInterviews) * 100
      : 0;

    const offerToHire = totalOffers > 0
      ? (totalHires / totalOffers) * 100
      : 0;

    const overallFunnel = totalApplications > 0
      ? (totalHires / totalApplications) * 100
      : 0;

    const byStage = this.calculateStageConversions(applications);

    const bySource = this.calculateSourceConversions(applications, interviews, offers, hiredApplications);
  ...
  private calculateStageConversions(applications: Application[]) {
    const stages = [
      { from: ApplicationStatus.NEW, to: ApplicationStatus.UNDER_REVIEW, name: 'New to Review' },
      { from: ApplicationStatus.UNDER_REVIEW, to: ApplicationStatus.SHORTLISTED, name: 'Review to Shortlist' },
      { from: ApplicationStatus.SHORTLISTED, to: ApplicationStatus.INTERVIEW_SCHEDULED, name: 'Shortlist to Interview' },
    ];

    return stages.map(stage => {
      const fromCount = applications.filter(app => app.status === stage.from).length;
      const toCount = applications.filter(app => 
        app.statusHistory?.some(h => h.status === stage.from && 
          app.statusHistory?.find(h2 => h2.status === stage.to && h2.changedAt > h.changedAt))
      ).length;
  ...
  private calculateSourceConversions(
    ...
    const sourceMap = new Map<string, { applications: number; interviews: number; offers: number; hires: number }>();

    applications.forEach(app => {
      const source = app.source || ApplicationSource.OTHER;
      if (!sourceMap.has(source)) {
        sourceMap.set(source, { applications: 0, interviews: 0, offers: 0, hires: 0 });
      }
    ...
    return Array.from(sourceMap.entries()).map(([source, data]) => ({
      source,
      applicationToInterview: data.applications > 0 ? (data.interviews / data.applications) * 100 : 0,
      interviewToOffer: data.interviews > 0 ? (data.offers / data.interviews) * 100 : 0,
  ...
  private calculateSourceEffectiveness(
    ...
    applications.forEach(app => {
      ...
      sourceStats.get(source)!.matchingScores.push(Number(app.matchingScore || 0));
    });
    ...
    const sources = Array.from(sourceStats.entries()).map(([source, stats]) => ({
      ...
      hireRate: stats.applications.length > 0
        ? (stats.hires.length / stats.applications.length) * 100
        : 0,
      averageMatchingScore: stats.matchingScores.length > 0
        ? stats.matchingScores.reduce((sum, score) => sum + score, 0) / stats.matchingScores.length
        : 0,
      averageTimeToHire: stats.timeToHire.length > 0
        ? stats.timeToHire.reduce((sum, days) => sum + days, 0) / stats.timeToHire.length
        : 0,
    }));
  ...
  private calculatePipelineMetrics(applications: Application[]): PipelineMetrics {
      ...
      if (app.pipelineStageHistory && app.pipelineStageHistory.length > 0) {
        ...
        for (let i = 0; i < history.length - 1; i++) {
          ...
          const days = Math.floor(
            (new Date(to.changedAt).getTime() - new Date(from.changedAt).getTime()) / (1000 * 60 * 60 * 24)
          );
          stageMap.get(stageKey)!.transitions.push({ from: from.stageKey, to: to.stageKey, days });
        }
      }
    });

    const stageMetrics = Array.from(stageMap.entries()).map(([stageKey, data]) => {
      const avgTime = data.transitions.length > 0
        ? data.transitions.reduce((sum, t) => sum + t.days, 0) / data.transitions.length
        : 0;
      ...
      const conversionToNext = data.applications.length > 0
    ...
    const bottlenecks = stageMetrics
      ...
      .map(stage => ({
        ...
        reason: stage.averageTimeInStage > 7
  ...
  private calculateQualityMetrics(
    ...
    const averageMatchingScore = allMatchingScores.length > 0
      ? allMatchingScores.reduce((sum, score) => sum + score, 0) / allMatchingScores.length
      : 0;

    const averageMatchingScoreOfHires = hiredMatchingScores.length > 0
      ? hiredMatchingScores.reduce((sum, score) => sum + score, 0) / hiredMatchingScores.length
      : 0;

    const acceptedOffers = offers.filter(o => o.status === OfferStatus.ACCEPTED).length;
    const offerAcceptanceRate = offers.length > 0
      ? (acceptedOffers / offers.length) * 100
      : 0;
    ...
    const averageInterviewsPerHire = interviewsPerHire.length > 0
      ? interviewsPerHire.reduce((sum, count) => sum + count, 0) / interviewsPerHire.length
      : 0;

    return {
      averageMatchingScore: Math.round(averageMatchingScore * 10) / 10,
      averageMatchingScoreOfHires: Math.round(averageMatchingScoreOfHires * 10) / 10,
  ...
  private calculateChanges(
    ...
    const timeToHireChange = current.timeMetrics.average - previous.timeMetrics.average;
    const timeToHirePercentage = previous.timeMetrics.average > 0
      ? (timeToHireChange / previous.timeMetrics.average) * 100
      : 0;

    const conversionRateChange = current.conversionRates.overallFunnel - previous.conversionRates.overallFunnel;
    const conversionRatePercentage = previous.conversionRates.overallFunnel > 0
      ? (conversionRateChange / previous.conversionRates.overallFunnel) * 100
      : 0;

    const totalHiresChange = current.overview.totalHires - previous.overview.totalHires;
    const totalHiresPercentage = previous.overview.totalHires > 0
