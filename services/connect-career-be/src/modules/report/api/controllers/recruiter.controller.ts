...
import { CreateReportDto, ReportListQueryDto } from '../dtos/report.dto';
import { Report } from '../../domain/entities/report.entity';
import { ReportableEntityType } from '../../domain/entities/report.entity';

@ApiTags('Reports - Recruiter')
@Controller('v1/recruiters/reports')
@UseGuards(JwtAuthGuard)
@ApiBearerAuth()
export class RecruiterReportController {
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        entityType: {
          type: 'string',
          enum: Object.values(ReportableEntityType),
        },
  ...
  getReasons(@Param('entityType') entityType: ReportableEntityType) {
    const reasons = this.reportService.getReasonsForEntityType(entityType);
