...
# Config
DATA_DIR = Path("D:/pet-project/connect-career-be/services/ai-service/data/crawler/career-viet")
LINKS_FILE = DATA_DIR / "career_viet_links.csv"
OUTPUT_FILE = DATA_DIR / "career_viet_jobs.json"
...
def main():
    try:
        with create_driver() as driver:
            logging.info("Starting CareerViet crawler...")
            crawler = career_viet_jobscrawler(driver=driver, file_path=str(LINKS_FILE))

            results = crawler.crawl()
            logging.info("Crawling finished, total jobs: %d", len(results))

            crawler.write_file(results, str(OUTPUT_FILE))
