...
export abstract class BaseUnitOfWork {
    protected readonly _repositories: Map<string, IRepository<any, any>> = new Map();
    ...
    private async processBeforeCommitEvents(): Promise<void> {
        ...
        const trackedEntities = Array.from(this._repositories.values())
            .flatMap(repo => repo.getTrackedEntities());
