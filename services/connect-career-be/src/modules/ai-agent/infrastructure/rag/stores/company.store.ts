...
@Injectable()
export class CompanyStore implements VectorStore {
  private readonly logger = new Logger(CompanyStore.name);
  ...
  async addDocuments(chunks: DocumentChunk[]): Promise<void> {
    ...
    this.logger.log(`Added ${chunks.length} company documents to store`);
  ...
  async deleteDocuments(ids: string[]): Promise<void> {
    ...
    this.logger.log(`Deleted ${ids.length} documents from company store`);
    return Promise.resolve();
  }
