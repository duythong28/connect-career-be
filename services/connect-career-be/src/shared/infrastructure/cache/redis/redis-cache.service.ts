...
export class RedisCacheService implements IDistributedCache {
  ...
  async stringSet<T>(
    ...
    try {
      ...
      if (options?.ttl) {
        return (
          (await this.redis.setex(redisKey, options.ttl, redisValue)) === 'OK'
        );
      }
  ...
  async setAdd<T>(key: string, member: T): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Failed to add to set ${key}: ${error.message}`,
        error.stack,
      );
      return false;
    }
  ...
  async setAddMany<T>(key: string, members: T[]): Promise<number> {
    try {
      ...
      this.logger.error(
        `Failed to add multiple to set ${key}: ${error.message}`,
        error.stack,
      );
      return 0;
    }
  ...
  async setMembers<T>(key: string): Promise<T[]> {
    try {
      ...
      this.logger.error(
        `Failed to get set members ${key}: ${error.message}`,
        error.stack,
      );
      return [];
    }
  ...
  async setPop<T>(key: string, count: number): Promise<T[]> {
    try {
      ...
      this.logger.error(
        `Failed to pop from set ${key}: ${error.message}`,
        error.stack,
      );
      return [];
    }
  ...
  async setRandomMembers<T>(key: string, count: number): Promise<T[]> {
    try {
      const members = await this.redis.srandmember(
        this.getRedisKey(key),
        count,
      );
      return RedisValueHelper.fromRedisValues<T>(
      ...
      this.logger.error(
        `Failed to get random members ${key}: ${error.message}`,
        error.stack,
      );
      return [];
    }
  ...
  async setRemove<T>(key: string, member: T): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Failed to remove from set ${key}: ${error.message}`,
        error.stack,
      );
      return false;
    }
  ...
  async sortedSetAdd<T>(
    ...
    try {
      ...
      this.logger.error(
        `Failed to add to sorted set ${key}: ${error.message}`,
        error.stack,
      );
      return false;
    }
  ...
  async sortedSetAddMany<T>(
    ...
    try {
      ...
      this.logger.error(
        `Failed to add multiple to sorted set ${key}: ${error.message}`,
        error.stack,
      );
      return false;
    }
  ...
  async sortedSetRemoveRangeByScore(
    key: string,
    min: number,
    max: number,
  ): Promise<number> {
  ...
  async sortedSetRangeByRank<T>(
    ...
    try {
      ...
      this.logger.error(
        `Failed to get range from sorted set ${key}: ${error.message}`,
        error.stack,
      );
      return [];
    }
